...
function destroy(
    ...
    $files = finder()
        ->in(variable('root_dir') . '/infra/docker/services/router/certs/')
        ->name('*.pem')
        ->files()
    ;
    fs()->remove($files);
...
function generate_certificates(
    ...
    $sslDir = variable('root_dir') . '/infra/docker/services/router/certs';
    ...
    if ($force) {
        if (file_exists($f = "{$sslDir}/cert.pem")) {
            io()->comment('Removing existing certificates in infra/docker/services/router/certs/*.pem.');
            unlink($f);
        }
    ...
    run(['infra/docker/services/router/generate-ssl.sh'], quiet: true);

    io()->success('Successfully generated self-signed SSL certificates in infra/docker/services/router/certs/*.pem.');
...
function docker_compose(array $subCommand, ?Context $c = null, bool $withBuilder = false, array $profiles = []): Process
{
    ...
    if ($withBuilder) {
        $command[] = '-f';
        $command[] = variable('root_dir') . '/infra/docker/docker-compose.builder.yml';
    }
